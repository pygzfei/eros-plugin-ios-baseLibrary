...
- (void)viewDidLoad {
    ...
    if (authStatus == AVAuthorizationStatusDenied) {
        ...
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Can't use camera" message:@"Please allow camera access in iPhone settings privacy camera" preferredStyle:UIAlertControllerStyleAlert];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(.02 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [SVProgressHUD showWithStatus:@"Scanner initializing..."];
    });
}
